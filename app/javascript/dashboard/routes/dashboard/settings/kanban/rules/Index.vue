...
watch(
  ...
  async (newBoardId) => {
    // Prevenir fechamento do Dialog durante a busca de colunas
...
const handleDialogClose = () => {
  // Prevenir fechamento acidental durante operações assíncronas
  if (isDialogClosing.value || isLoadingColumns.value) {
    return;
  }
  
  // Só fecha se realmente for uma ação de fechar, não por interações dentro do form
...
<template>
  <SettingsLayout
    ...
    :no-records-found="!rules.length && !isLoading"
    :no-records-message="t('KANBAN.RULES_EMPTY')"
  >
    <template #header>
      <BaseSettingsHeader
        :title="t('KANBAN.RULES_TITLE')"
        :description="t('KANBAN.RULES_DESCRIPTION')"
      >
        <template #actions>
          <Button
            icon="i-lucide-plus"
    ...
    <template #body>
      <div class="grid gap-6">
        ...
        <div class="p-6 bg-n-solid-2 border border-n-weak rounded-lg">
          ...
          <div v-if="!rules.length" class="py-8 text-sm text-center text-n-slate-11">
