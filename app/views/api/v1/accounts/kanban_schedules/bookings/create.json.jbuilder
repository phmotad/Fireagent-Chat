...
if @booking.kanban_card
  json.kanban_card do
    json.id @booking.kanban_card.id
    json.title @booking.kanban_card.title
  end
end

# Informações de ocupação após criar/atualizar o booking
if @occupancy_data
  json.occupancy do
    json.max_capacity @occupancy_data[:max_capacity]
    json.booked_count @occupancy_data[:booked_count]
    json.available_slots @occupancy_data[:available_slots]
    json.is_full @occupancy_data[:is_full]
  end
end
